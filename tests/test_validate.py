...
from validatehttp.spec import ValidatorSpecRule
from validatehttp.spec import YamlValidatorSpec
from validatehttp.validate import ValidationFail
from validatehttp.validate import ValidationPass
...
class TestValidatorValidation(TestCase):
    ...
    def test_response_status_mismatch(self, mock):
        ...
        mock.return_value.headers = {"x-test": "foobar"}
        for result in self.validator.validate():
            self.assertRuleMatches(result, passing=False, error=r"mismatch: status_code")
    ...
    def test_response_header_mismatch(self, mock):
        ...
        for result in self.validator.validate():
            self.assertRuleMatches(result, passing=False, error=r"header mismatch: x-test")
